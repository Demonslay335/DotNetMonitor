...
namespace DotNetMonitor.Controls.HexBox
{
    ...
    public class HexBox : Control
    {
        ...
        class KeyInterpreter : IKeyInterpreter
        {
            ...
            protected virtual bool PreProcessWmKeyDown_ShiftUp(ref Message m)
            {
                ...
                if (_bpiStart.Index >= pos + sel)
                {
                    if (pos == 0)
                        return true;
                    pos = Math.Max(0, pos - _hexBox._iHexMaxHBytes);
                    if (pos == 0)
                        sel = _bpiStart.Index;
                    else
                ...
                {
                    if (sel - _hexBox._iHexMaxHBytes < 0)
                    {
                        pos = Math.Max(0, pos - _hexBox._iHexMaxHBytes);
                        if (pos == 0)
                            sel = _bpiStart.Index;
            ...
            protected virtual bool PreProcessWmKeyDown_ShiftDown(ref Message m)
            {
                ...
                if (_bpiStart.Index <= pos)
                {
                    sel += _hexBox._iHexMaxHBytes;
                    if (pos + sel > _hexBox._byteProvider.Length)
                        sel = _hexBox._byteProvider.Length - pos;
                    _hexBox.InternalSelect(pos, sel);
                    _hexBox.ScrollByteIntoView(pos + sel);
                }
                else
            ...
            protected virtual bool PreProcessWmKeyDown_ControlHome(ref Message m)
            {
                long pos = _hexBox._bytePos;
                int cp = _hexBox._byteCharacterPos;
            ...
            protected virtual bool PreProcessWmKeyDown_ControlEnd(ref Message m)
            {
                long pos = _hexBox._bytePos;
                int cp = _hexBox._byteCharacterPos;
            ...
            protected virtual bool PreProcessWmKeyDown_Escape(ref Message m)
            {
                _hexBox.ReleaseSelection();
                return true;
            }
            ...
            protected virtual bool PreProcessWmKeyDown_Home(ref Message m)
            {
                ...
                long begin = (long)Math.Floor(pos / (double)_hexBox._iHexMaxHBytes) * _hexBox._iHexMaxHBytes;
            ...
            protected virtual bool PreProcessWmKeyDown_End(ref Message m)
            {
                ...
                long end = (long)Math.Floor(pos / (double)_hexBox._iHexMaxHBytes) * _hexBox._iHexMaxHBytes + _hexBox._iHexMaxHBytes - 1;
                end = Math.Min(_hexBox._byteProvider.Length, end);
                ...
                get
                {
                    if (_messageHandlers == null)
                    {
                        _messageHandlers = new Dictionary<Keys, MessageDelegate>
                        {
                            ...
                            { Keys.Home | Keys.Control, new MessageDelegate(PreProcessWmKeyDown_ControlHome) }, // move to home of stream
                            { Keys.End | Keys.Control, new MessageDelegate(PreProcessWmKeyDown_ControlEnd) }, // move to end of stream
                            ...
                            { Keys.A | Keys.Control, new MessageDelegate(PreProcessWmKeyDown_ControlA) }, // select all
                            { Keys.Escape, new MessageDelegate(PreProcessWmKeyDown_Escape) }, // cancel selection
                            { Keys.Home, new MessageDelegate(PreProcessWmKeyDown_Home) }, // move to beginning of current row
                            { Keys.End, new MessageDelegate(PreProcessWmKeyDown_End) } // move to end of current row
                        };
                    }
        ...
        /// <summary>
        /// Occurs, when the underlying ByteProvider buffer has changed.
        /// </summary>
        [Description("Occurs, when the underlying ByteProvider buffer has changed.")]
        ...
        public void VScrollBar_Scroll(ScrollEventArgs e)
        {
            _vScrollBar_Scroll(null, e);
        }
        ...
        void _vScrollBar_Scroll(object sender, ScrollEventArgs e)
        {
            switch (e.Type)
            {
                ...
                case ScrollEventType.ThumbTrack:
                    ...
                    break;
                /* This causes a delay with synced hexboxes, where one will be one event behind until EndScroll
                case ScrollEventType.ThumbTrack:
                    ...
                    if (currentThumbTrack - _lastThumbtrack > THUMBTRACKDELAY)
                    {
        ...
        public bool CaretIsFocusedHex()
        {
            return Caret.GetPos(out Point p) && _recHex.Contains(p);
        }
        #endregion
        ...
        public void GoToOffset(GotoOptions options)
        {
            switch (options.Relative)
            {
                case GotoOffsetRelative.Current:
                    options.ByteIndex = _bytePos - options.ByteIndex;
                    break;
                case GotoOffsetRelative.End:
                    options.ByteIndex = this.ByteProvider.Length - options.ByteIndex;
                    break;
            }

            SetPosition(options.ByteIndex);
        ...
        public void Copy()
        {
            ...
            da.SetData(DataFormats.StringFormat, sBuffer);
...
//            da.SetData("BinaryData", ms);
        ...
        public void Paste()
        {
            ...
            else if (da.GetDataPresent(DataFormats.StringFormat))
            {
                string sBuffer = (string)da.GetData(DataFormats.StringFormat);
        ...
        /// <summary>
        /// Return true if CutHex method could be invoked.
        /// </summary>
        public bool CanCutHex()
        {
            if (ReadOnly || !this.Enabled)
                return false;
            if (_byteProvider == null)
        ...
        public bool CanPasteHex()
        {
            ...
            byte[] buffer = null;
            IDataObject da = Clipboard.GetDataObject();
            if (da.GetDataPresent(DataFormats.StringFormat))
            {
                string hexString = (string)da.GetData(DataFormats.StringFormat);
        ...
        public void CopyHex()
        {
            ...
            da.SetData(DataFormats.StringFormat, hexString);
...
//            da.SetData("BinaryData", ms);
        ...
        protected virtual void PaintHex(Graphics g, long startByte, long endByte)
        {
            Brush brush = new SolidBrush(GetDefaultForeColor());
            Brush asciiBrush = new SolidBrush(_asciiForeColor);
            Brush newlineBrush = new SolidBrush(_newlineForeColor);
            Brush zeroBrush = new SolidBrush(_zeroForeColor);
            ...
            for (long i = startByte; i < intern_endByte + 1; i++)
            {
                ...
                {
                    peeked_before = true;
                    prev_b = _byteProvider.ReadByte(i - 1);
                }
                if (i + 1 < intern_endByte + 1)
                {
                    peeked_ahead = true;
                    next_b = _byteProvider.ReadByte(i + 1);
                }                

                // Check for selected byte
                ...
                {
                    PaintHexString(g, b, asciiBrush, gridPoint);
                }
                // Check for newline sequence
                else if((peeked_before && prev_b == 0x0D && b == 0x0A) || (peeked_ahead && next_b == 0x0A && b == 0x0D))
                {
                    PaintHexString(g, b, newlineBrush, gridPoint);
                }
                // Check for zero byte
                else if(b == 0x00)
                {
                    PaintHexString(g, b, zeroBrush, gridPoint);
                }
                else
                {
        ...
        protected void PaintHexString(Graphics g, byte b, Brush brush, Point gridPoint)
        {
        ...
        protected virtual void PaintHexAndStringView(Graphics g, long startByte, long endByte)
        {
            Brush brush = new SolidBrush(GetDefaultForeColor());
            Brush asciiBrush = new SolidBrush(_asciiForeColor);
            Brush newlineBrush = new SolidBrush(_newlineForeColor);
        ...
        protected Point GetGridBytePoint(long byteIndex)
        {
        ...
        public IByteProvider ByteProvider
        {
            ...
            set
            {
                ...
                if (_byteProvider != null)
                {
                    ...
                    _byteProvider.Changed -= new EventHandler(_byteProvider_Changed);
                }
        ...
        protected IByteProvider _byteProvider;
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the foreground color for ASCII bytes
        /// </summary>
        [DefaultValue(typeof(Color), "Black"), Category("Hex"), Description("Gets or sets the foreground color for ASCII bytes.")]
        public Color AsciiForeColor
        {
            get { return _asciiForeColor; }
            set { _asciiForeColor = value; Invalidate(); }
        }
        Color _asciiForeColor = Color.Black;

        /// <summary>
        /// Gets or sets the foreground color for newline bytes
        /// </summary>
        [DefaultValue(typeof(Color), "Black"), Category("Hex"), Description("Gets or sets the foreground color for newline bytes.")]
        public Color NewLineForeColor
        {
            get { return _newlineForeColor; }
            set { _newlineForeColor = value; Invalidate(); }
        }
        Color _newlineForeColor = Color.Black;

        /// <summary>
        /// Gets or sets the foreground color for 0x00 bytes
        /// </summary>
        [DefaultValue(typeof(Color), "Black"), Category("Hex"), Description("Gets or sets the foreground color for 0x00 bytes.")]
        public Color ZeroForeColor
        {
            get { return _zeroForeColor; }
            set { _zeroForeColor = value; Invalidate(); }
        }
        Color _zeroForeColor = Color.Black;
        ...
        /// <summary>
        /// Gets or sets the delegated status label
        /// </summary>
        [Description("Gets or sets the delegated status label")]
        public ToolStripStatusLabel DelegatedStatusLabel { get; set; }
        ...
        /// <summary>
        /// Raises the BytesChanged event.
        /// </summary>
        /// <param name="e">An EventArgs that contains the event data.</param>
        protected virtual void OnBytesChanged(EventArgs e)
        {
        ...
        void _byteProvider_Changed(object sender, EventArgs e)
        {
            OnBytesChanged(e);
        }
        #endregion
